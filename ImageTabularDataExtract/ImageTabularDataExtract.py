...
class ImageTextExtract:
    ...
    def imageToExcelMain(self):
        ...
        root.title("Extract tabular data from image")
        ...
        lbl1 = tk.Label(frame, text='Export Tabular Data From Image', font=('comicsansms',15,'bold') )
        lbl1.place(relx=0.15, rely=0.05, relwidth=0.70, relheight=0.10)

        lbl2 = tk.Label(frame, text='Enter Image path: ', font=('comicsansms',9,'bold'))
        lbl2.place(relx=0.02, rely=0.2, relwidth=0.20, relheight=0.1)

        lbl3 = tk.Label(frame, text='Enter Excel path: ', font=('comicsansms',9,'bold'))
        ...
        imgFilePath = tk.StringVar(frame)
        imgFilePathEntry = tk.Entry(frame, textvariable=imgFilePath, font=('comicsansms',8))
        imgFilePathEntry.place(relx=0.25, rely=0.2, relwidth=0.72, relheight=0.1)

        excelFilePath = tk.StringVar(frame)
        excelFilePathEntry = tk.Entry(frame, textvariable=excelFilePath, font=('comicsansms',8))
        excelFilePathEntry.place(relx=0.25, rely=0.35, relwidth=0.72, relheight=0.1)

        btn = tk.Button(root, text="Export to Excel", bg='#ffa64d',command=btnClick, font=('comicsansms',14,'bold'))
        btn.place(relx=0.3, rely=0.55, relheight=0.1, relwidth=0.3)

        def getImagePath():
            imgPath = imgFilePath.get()
            return imgPath

        def getExcelPath():
            excelPath = excelFilePath.get()
            return excelPath
        ...
        def exportToExcel():
            ...
            try:
                ...
                # read your file
                extension = os.path.splitext(imgPath)
                ...
                # inverting the image
                img_bin = 255 - img_bin
                # cv2.imwrite(extension[0] + r'\cv_inverted.png', img_bin)
                ...
                # cv2.imwrite(extension[0] + r'\vertical.jpg', vertical_lines)
                ...
                # cv2.imwrite(extension[0] + r'\horizontal.jpg', horizontal_lines)
                # Plot the generated image
                ...
                # cv2.imwrite(extension[0] + r'\img_vh.jpg', img_vh)
                ...
                dataframe.to_excel(excelPath)

                labelLowerFrame['text'] = 'Image to text exported \n successfully'
...
if __name__ == '__main__':
    imgTextExtract = ImageTextExtract()
    imgTextExtract.imageToExcelMain()
